...
async function compute() {
  if (!G || !startId || !goalId) { currentPath = []; renderRouteInfo(); draw(); return; }
  const algo = document.getElementById('algo').value;

  const res = await fetch('/api/route', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({ start: startId, end: goalId, algo })
  });
  if (!res.ok) throw new Error('POST /api/route failed ' + res.status);
